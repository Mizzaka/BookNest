...
const BranchTable = ({ data, columns, onEdit, onDeleteSuccess }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/announcements/${selectedRowId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }, // Add token if required
      });
      console.log("Deleted Announcement with ID:", selectedRowId);
      onDeleteSuccess(selectedRowId); // Notify parent to update state
      setDeleteDialogOpen(false);
      alert("Announcement deleted successfully!");
    } catch (error) {
      console.error("Error deleting announcements:", error.response?.data || error.message);
    }
  };
