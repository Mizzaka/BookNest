...
const BranchTable = ({ data, columns, onEdit, onDeleteSuccess }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/api/branches/${selectedRowId}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }, // Add token if required
      });
      console.log("Deleted branches with ID:", selectedRowId);
      onDeleteSuccess(selectedRowId); // Notify parent to update state
      setDeleteDialogOpen(false);
      alert("Branche deleted successfully!");
    } catch (error) {
      console.error("Error deleting branche:", error.response?.data || error.message);
    }
  };
  ...
  return (
    <>
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete this branche? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
